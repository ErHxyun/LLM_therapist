...
class IntermissionRunnerTests(unittest.TestCase):
    def test_private_screening_answer_stays_in_memory_and_bridges_when_ready(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            runner = IntermissionRunner(
                IntermissionSettings(
                    ...
                    bridge_text="Caiti is ready now.",
        ...
        self.assertEqual(spoken[-1], "Caiti is ready now.")
    ...
    def test_scripted_task_runs_without_stt_or_main_record_dependencies(self):
        ...
        self.assertIn("Caiti needs a little time to think", spoken[0])
        self.assertIn("private check-in", spoken[0])
        self.assertIn("Simple breath awareness", spoken[1])
        self.assertIn("rhythm of breathing", spoken[1])
        self.assertEqual(spoken[2], "Back to Caiti.")
        self.assertEqual(led_events, [True, False, True, False, True, False])
    ...
    def test_intermission_cools_down_after_long_user_speech_trigger(self):
        ...
        self.assertEqual(len([text for text in spoken if "private check-in" in text.lower()]), 2)
    ...
    def test_intermission_requires_interval_counter_before_random_gate(self):
        ...
        self.assertEqual(runner.screening_answers["phq2_1"], 3)
        self.assertEqual(len([text for text in spoken if "private check-in" in text.lower()]), 1)
